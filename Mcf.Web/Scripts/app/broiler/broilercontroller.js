...
mcfapp.controller('broilercontroller', ['$scope', 'jobservice', 'broilerservice', 'uiGridConstants', 'commonservice', broilercontroller]);

function broilercontroller($scope, jobservice, broilerservice, uiGridConstants, commonservice) {
    $scope.gridOptions = [];
    $scope.UpdateGridData = function (rowEntity) {
        ...
        broilerservice.saveData({}, dsUpdatedData).$promise.then(function (data) {
        }, function (error) {
        });
    };
    ...
    $scope.GetCurrentJobInfo = function (datasource) {
        ...
        $scope.broilerjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
        $scope.broilerjobinfo.$promise.then(function (data) {
            $scope.broilerjobinfo = data[0];
            $scope.broilerjobinfo.startTime = FormatDate($scope.broilerjobinfo.startTime);
            $scope.broilerjobinfo.endTime = FormatDate($scope.broilerjobinfo.endTime);
            $scope.GetRawData();
        }, function (error) {
            unblockUI();
        });
    ...
    $scope.GetRawData = function () {
        $('#table-container').jexcel({
            // Full CSV URL
            csv: '/DataFiles/Broiler/brls_all_tables.csv',
    ...
    $scope.GetBroilerFormattedData = function () {
        ...
        if (to != '')
            index = 0;
        $scope.broilerformatteddata = broilerservice.getBroilerFormattedData({ index: index, from: from, to: to });
        $scope.broilerformatteddata.$promise.then(function (data) {
    ...
    $scope.addData = function (griddata, commodity) {
        var newRow = {};
        if (griddata != undefined) {
            $.each(griddata.columnDefs, function (key, value) {               
                newRow.name = value.name;
                newRow.displayName = value.name;
                newRow.field = value.field;
                newRow.enableCellEdit = true;
            })
        }
        if (commodity != undefined && commodity.length) {
            var obj = $.grep($scope.gridOptions, function (obj) { return obj.header.name === commodity; })[0];
            obj.push(newRow);
        }
        else {
            $scope.gridOptions[0].columnDefs.push(newRow);
            $scope.gridOptions[0].data.push(newRow);
        }
    }
