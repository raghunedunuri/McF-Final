...
mcfapp.controller('sugarcontroller', ['$scope', '$resource', 'jobservice', 'sugarservice', 'uiGridConstants', 'commonservice', sugarcontroller]);
function sugarcontroller($scope, $resource, jobservice, sugarservice, uiGridConstants, commonservice) {
    $scope.gridOptions = [];
    $scope.GetCurrentJobInfo = function (datasource) {
        blockUI();
        $scope.datasource = datasource;
        $scope.sugarjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
        $scope.sugarjobinfo.$promise.then(function (data) {
            $scope.sugarjobinfo = data[0];
            if ($scope.sugarjobinfo != undefined)
                $scope.sugarjobinfo.startTime = FormatDate($scope.sugarjobinfo.startTime);
            $scope.GetRawData();
        }, function (error) {
            unblockUI();
        });
    ...
    $scope.GetSugarFormattedData = function () {
        ...
        if (to != '')
            index = 0;
        $scope.sugarformatteddata = sugarservice.getFormattedData({ index: index, from: from, to: to });
        $scope.sugarformatteddata.$promise.then(function (data) {
            ...
            for (var key in obj) {
                if (obj.hasOwnProperty(key)) {
                    ...
                    for (var j = 0; j < $scope.griddata[key].headers.length; j++) {
                        var units = $scope.griddata[key].headers[j].unit != undefined ? '(' + $scope.griddata[key].headers[j].unit + ')' : '';
	...
	$scope.GetRawData = function () {
		$('#table-container').jexcel({
			// Full CSV URL
		    csv: '/DataFiles/Sugar/Copy of all_smd_tables.csv',
