...
mcfapp.controller('dtncontroller', ['$scope', 'jobservice', 'dtnservice', 'uiGridConstants', 'commonservice', dtncontroller]);

function dtncontroller($scope, jobservice, dtnservice, uiGridConstants, commonservice) {
    $scope.gridOptions = [];
    ...
    $scope.GetCurrentJobInfo = function (datasource) {
        ...
        $scope.dtnjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
        $scope.dtnjobinfo.$promise.then(function (data) {
            $scope.dtnjobinfo = data[0];
            $scope.dtnjobinfo.startTime = FormatDate($scope.dtnjobinfo.startTime);
            $scope.dtnjobinfo.endTime = FormatDate($scope.dtnjobinfo.endTime);
            $scope.GetDTNFormattedData();
        }, function (error) {
            unblockUI();
        });
    }
    ...
    $scope.GetDTNFormattedData = function () {
        ...
        if (to != '')
            index = 0;
        $scope.dtnformatteddata = dtnservice.getDTNFormattedData({ index: index, from: from, to: to });
        $scope.dtnformatteddata.$promise.then(function (data) {
    ...
    $scope.getTableHeight = function () {
        ...
        var headerHeight = 35; // header height
        return {
