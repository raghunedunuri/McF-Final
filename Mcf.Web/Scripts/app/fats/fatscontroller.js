...
mcfapp.controller('fatscontroller', ['$scope', '$resource', 'jobservice', 'fatsservice', 'uiGridConstants', 'commonservice', fatscontroller]);
function fatscontroller($scope, $resource, jobservice, fatsservice, uiGridConstants, commonservice) {
    $scope.gridOptions = [];

    $scope.GetCurrentJobInfo = function (datasource) {
        //blockUI();
        $scope.datasource = datasource;
        $scope.fatsjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
        $scope.fatsjobinfo.$promise.then(function (data) {
            $scope.fatsjobinfo = data[0];
            if ($scope.fatsjobinfo != undefined)
                $scope.fatsjobinfo.startTime = FormatDate($scope.fatsjobinfo.startTime);
            $scope.GetRawData();
        }, function (error) {
            unblockUI();
        });
    ...
    $scope.GetFatsFormattedData = function () {
        ...
        if (to != '')
            index = 0;
        $scope.fatsformatteddata = fatsservice.getFatsFormattedData({ Index: index, From: from, To: to });
        $scope.fatsformatteddata.$promise.then(function (data) {
            ...
            for (var key in obj) {
                ...
                $scope.commodity.name = $scope.gridOptions[0].header.name;
            }
            unblockUI();
        }, function (error) {
            unblockUI();
    ...
    $scope.GetRawData = function () {
        $('#table-container').jexcel({
            // Full CSV URL
            csv: '/DataFiles/Fats/cafo_all_tables.csv',
