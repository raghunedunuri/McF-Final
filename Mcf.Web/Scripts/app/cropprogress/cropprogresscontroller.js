...
mcfapp.controller('cropprogresscontroller', ['$scope', '$resource', 'jobservice', 'cropprogressservice', 'uiGridConstants', 'commonservice', cropprogresscontroller]);
function cropprogresscontroller($scope, $resource, jobservice, cropprogressservice, uiGridConstants, commonservice) {
    $scope.gridOptions = [];
    $scope.isOkToShow = false;
    $scope.GetCropFormattedData = function () {
        ...
        if (to != '')
            index = 0;
        $scope.cropformatteddata = cropprogressservice.getCropFormattedData({ Index: index, From: from, To: to });
        $scope.cropformatteddata.$promise.then(function (data) {
    ...
    $scope.GetCurrentJobInfo = function (datasource) {
        ...
        $scope.cropjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
        $scope.cropjobinfo.$promise.then(function (data) {
            $scope.cropjobinfo = data[0];
            if ($scope.cropjobinfo != undefined)
                $scope.cropjobinfo.startTime = FormatDate($scope.cropjobinfo.startTime);
            $scope.GetRawData();
        }, function (error) {
            unblockUI();
        });
    ...
    $scope.GetRawData = function () {
        $('#table-container').jexcel({
            // Full CSV URL
            csv: '/DataFiles/CropProgress/prog_all_tables.csv',
