...
mcfapp.controller('chickencontroller', ['$scope', '$resource', 'jobservice', 'uiGridConstants', 'commonservice', chickencontroller]);
function chickencontroller($scope, $resource, jobservice, uiGridConstants) {
	$scope.gridOptions = [];
	$scope.GetCurrentJobInfo = function (datasource) {	    
		$scope.chickenjobinfo = jobservice.getCurrentJobInfo({ datasource: datasource });
		$scope.chickenjobinfo.$promise.then(function (data) {
		    $scope.chickenjobinfo = data[0];
		    if ($scope.chickenjobinfo != undefined)
		        $scope.chickenjobinfo.startTime = FormatDate($scope.chickenjobinfo.startTime);
		    $scope.GetRawData();
		}, function (error) {
		    unblockUI();
		});
	...
	$scope.GetChickenFormattedData = function () {
	    ...
	    if (to != '')
	        index = 0;
	    $scope.chickenformatteddata = chickenservice.getFormattedData({ index: index, from: from, to: to });
	    $scope.chickenformatteddata.$promise.then(function (data) {
