...
mcfapp.controller('dataviewcontroller', ['$scope', '$resource', 'jobservice', 'dataviewservice', 'uiGridConstants', dataviewcontroller]);
function dataviewcontroller($scope, $resource, jobservice, dataviewservice, uiGridConstants) {
    $scope.gridOptions = [];
    $scope.datasource = '';
    $scope.rootsource = '';
    $scope.GetCurrentJobInfo = function (datasource, rootsource) {
        $scope.datasource = datasource;
        $scope.rootsource = rootsource;
        $scope.currentjobinfo = jobservice.getCurrentJobInfo({ datasource: $scope.datasource });
        $scope.currentjobinfo.$promise.then(function (data) {
            $scope.currentjobinfo = data[0];
            if ($scope.currentjobinfo != undefined)
                $scope.currentjobinfo.startTime = FormatDate($scope.currentjobinfo.startTime);
            //$scope.GetRawData();
            $scope.GetFormattedData();
        }, function (error) {
            unblockUI();
        });
    }
    ...
    $scope.GetFormattedData = function () {
        ...
        var from = $('#calendarStartDate')[0].value;
        var to = $('#calendarEndDate')[0].value;
        var index = $("#selduration option:selected").val();
        if (to != '')
            index = 0;
        $scope.formatteddata = dataviewservice.getFormattedData({ datasource: $scope.datasource, rootsource: $scope.rootsource, index: index, from: from, to: to });
        $scope.formatteddata.$promise.then(function (data) {
    ...
    $scope.GetRawData = function () {
        $('#table-container').jexcel({
            // Full CSV URL
            csv: '/DataFiles/ChickenAndEggs/ckeg_all_tables.csv',
            // Use the first row of your CSV as the headers
            csvHeaders: false,
            tableOverflow: true,
            // Headers
            colWidths: [70, 200, 300, 100, 100, 100, 100, 100, 100, 100, 70, 200, 300, 100, 100, 100, 100, 100, 100, 100],
        });
