...
namespace McKeany
{
    internal static class CornCommon
    {
        ...
        public static void InitConfigData(TreeView treeGroups)
        {
            ...
            WASDEDomesticConfigData = commonRepo.ExecuteDataSetFromSP("McF_GET CORN_CONFIG");
            ...
            {
                string comm = dr["Source"].ToString();
                string Market = dr["MarketName"].ToString();

                if (!dictGroups.ContainsKey(comm))
                {
                    dictGroups.Add(comm, new List<String>() );
                }
                dictGroups[comm].Add(Market);
            }

          
            foreach(KeyValuePair<string, List<String>> kv in dictGroups)
            {
