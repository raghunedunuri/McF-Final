...
namespace McKeany
{
    internal static class CocoaSDCommon
    {
        ...
        public static void InitConfigData(TreeView treeGroups, TreeView treeFields)
        {
            ...
            WASDEDomesticConfigData = commonRepo.ExecuteDataSetFromSP("McF_GET COCOASD_CONFIG");

            Dictionary<string, List<String>> dictGroups = new Dictionary<string, List<String>>();

            TreeNode Node = treeGroups.Nodes.Add("Production");
            foreach (DataRow dr in WASDEDomesticConfigData.Tables[0].Rows)
            {
                Node.Nodes.Add(dr["REGION"].ToString());
            }
            Node = treeGroups.Nodes.Add("Stocks");
            foreach (DataRow dr in WASDEDomesticConfigData.Tables[1].Rows)
            {
                Node.Nodes.Add(dr["REGION"].ToString());
            }
            Node = treeGroups.Nodes.Add("Grind");
            foreach (DataRow dr in WASDEDomesticConfigData.Tables[2].Rows)
            {
                Node.Nodes.Add(dr["REGION"].ToString());
            }
        }
