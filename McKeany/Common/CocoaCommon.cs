...
namespace McKeany
{
    internal static class CocoaCommon
    {
        ...
        static CocoaCommon()
        {
            ...
            DataFields.Add("Commodity_Code");
            DataFields.Add("Report_Date");
            DataFields.Add("Quarter_EndingDate");
            DataFields.Add("Source_Info");
            DataFields.Add("Region_Name");
            DataFields.Add("Stock_MeasureUnits");
            DataFields.Add("Cocoa_Grind");
        }
        ...
        public static string GetDateQuery(string dateQuery)
        {
            ...
            return DataOperations.GetDateQuery(Convert.ToInt32(filters[0]), filters[1], filters[2], "Report_Date", "COCOA_GRIND_DIALY_DATA");
        }
        ...
        public static void PresentData(Excel.Worksheet currentWorksheet, string Query)
        {
            ...
            if (ds != null)
            {
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    foreach (DataColumn dcol in ds.Tables[0].Columns)
                    {
                        if ( dcol.ColumnName.Trim().ToUpper() == "REPORT_DATE")
                        {
        ...
        public static string GetSelectedQuery(TreeView treeFields)
        {
            ...
            String Query = $"Select {Fields} from COCOA_GRIND_DIALY_DATA WHERE source_info in ({source})";
            return Query;
        }
        ...
        public static void InitConfigData(TreeView treeFields)
        {
            ...
            BHConfigInfo = commonRepo.ExecuteDataSetFromSP("McF_GET_COCOA_CONFIG");
            foreach(DataRow dr in BHConfigInfo.Tables[0].Rows)
            {
                treeFields.Nodes.Add(dr["Source_Info"].ToString());
            }
            treeFields.Nodes.Add("Origin");
        }
    }
}
