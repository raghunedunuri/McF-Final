...
namespace McKeany
{
    internal static class PCCommon
    {
        ...
        public static void InitConfigData(TreeView treeGroups)
        {
            ...
            WASDEDomesticConfigData = commonRepo.ExecuteDataSetFromSP("McF_GET_PC_DATA");

            Dictionary<string, bool> dictGroups = new Dictionary<string, bool>();
            Dictionary<string, bool> dictSubGroup = new Dictionary<string, bool>();
            Dictionary<string, List<string>> dictCommType = new Dictionary<string, List<string>>();
            Dictionary<string, bool> dictMarketType = new Dictionary<string, bool>();
            
            foreach (DataRow dr in WASDEDomesticConfigData.Tables[0].Rows)
            {
                string commType = dr["CommoditySubGroup1"].ToString();
                string displayName = dr["DisplayName"].ToString();

                if (!dictCommType.ContainsKey(commType))
                {
                    dictCommType.Add(commType, new List<string>());
                }
                dictCommType[commType].Add(displayName);
            }

            foreach (KeyValuePair<string, List<string>> kv in dictCommType)
            {
