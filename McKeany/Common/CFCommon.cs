...
namespace McKeany
{
    internal static class CFCommon
    {
        private static ICommonRepository commonRepo;
        private static DataSet CFConfigInfo;
        ...
        static CFCommon()
        {
            ...
            DataFields.Add("BeginDate");
            DataFields.Add("EndDate");
        ...
        public static string MergeTimeQuery(string query, string datequery)
        {
            ...
            string finalQuery = $"Select * from ( {query} ) A where ROWNUM = 1 Order by ReportDate";
            return finalQuery;
        }
        ...
        public static string GetDateQuery(string dateQuery)
        {
            ...
            return DataOperations.GetDateQuery(Convert.ToInt32(filters[0]), filters[1], filters[2], "ReportDate", "CATFEED_DIALY_DATA");
        }
        ...
        public static void PresentData(Excel.Worksheet currentWorksheet, string Query)
        {
            ...
            if (ds != null)
            {
                ...
                foreach (DataColumn dcol in ds.Tables[0].Columns)
                {
                    ...
                    DataRow[] dr = CFConfigInfo.Tables[0].Select($"Name = '{colName}'");
                    currentWorksheet.Cells[startrow + 1, column] = colName;

                    if (dr != null && dr.Length > 0)
                    {
                        currentWorksheet.Cells[startrow + 1, column] = dr[0]["DisplayName"]?.ToString(); ;
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    foreach (DataColumn dcol in ds.Tables[0].Columns)
                    {
                        ...
                        if (dcol.ColumnName.Trim().ToUpper() == "BEGINDATE" || 
                            dcol.ColumnName.Trim().ToUpper() == "ENDDATE" ||
                            dcol.ColumnName.Trim().ToUpper() == "REPORTDATE")
                        {
        ...
        public static string GetSelectedQuery(TreeView treeFields)
        {
            ...
            foreach (TreeNode node in treeFields.Nodes)
            {
                if (node.Checked)
                {
                    DataRow []dr = CFConfigInfo.Tables[0].Select($"DisplayName = '{node.Text}'");
                    Fields += $"{dr[0]["Name"].ToString()},";
                }
            }
            if (!String.IsNullOrEmpty(Fields))
                Fields = Fields.Substring(0, Fields.Length - 1);

            String Query = $"Select {Fields}, ROW_NUMBER () OVER (PARTITION BY BeginDate,Enddate ORDER BY ReportDate DESC ) ROWNUM from CATFEED_DIALY_DATA";
            return Query;
        }
        ...
        public static void InitConfigData(TreeView treeFields)
        {
            ...
            CFConfigInfo = commonRepo.ExecuteDataSetFromSP("McF_GET_CF_CONFIG");
            foreach (DataRow dr in CFConfigInfo.Tables[0].Rows)
            {
